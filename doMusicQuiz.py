...
def read_scores(file):
    d = {}
    s = open(file, 'r')
    print("The current top players and scores are:")
    for line in s:
        (score, user) = line.split()
        d[score] = user
    #for score, user in collections.OrderedDict(sorted(d.items(), key=lambda x: x[0], reverse=True)).items():
    #top_scores = s.read()
        print(score, user)
